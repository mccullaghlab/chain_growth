...
subroutine grow_the_chain(outFile)
        ...
        do chain = 1, nChains
               ...
               do monomer = 3, nMonomers
                ...
                enddo 
                open(26,file="temp.xyz")
                write(26,'(i10)') nMonomers
                write(26,'(i10)') nMonomers
                do monomer=1,nMonomers
                        write(26,'("C  ",4f12.4)') chainCoord(chain,monomer,1), chainCoord(chain,monomer,2),chainCoord(chain,monomer,3),sqrt(dot_product(chainCoord(chain,monomer,:),chainCoord(chain,monomer,:)))
                enddo
                close(26)
        enddo
        !$omp end do nowait
...
!        real (kind=8), parameter :: sigma12 = 68719476736.0
        ...
        ! pair energy
        if (nMonomers>3) then
                do monomer = 1, nMonomers-3 
                        diff = chainCoord(monomer,:) - newCoord
                        r2 = dot_product(diff,diff)
                        r6 = r2*r2*r2
                        r12 = r6*r6
                        newEnergy = newEnergy + eps * ( sigma12/r12 - sigma6/r6 )
...
subroutine compute_angle_energy(r1,r2,r3,energy)
        ...
        energy = 0.5 * ang_k * (theta - ang_eq)**2
